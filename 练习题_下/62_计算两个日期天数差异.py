...
#
# 3、提取间隔的天数:
# 可以用delta.days属性提取,delta 是 'datetime.timedelta' 类型

# 4、year1,month1,day1 = map(int,start_date.split('-'))
# 1）map的解包
# 2）map(int,列表) 输出是一个map对象，支持解包，作用相当于强制转换字符串类型为int类型----参数接收的参数都是int类型
# 3）year1,month1,day1 = start_date.split('-') 参数接收的是字符串类型
#
# 5、date(year1,month1,day1)
# 1）date类型对象的创建，用date(年，月，日)，其中年月日需是int类型
